/**
 * battle-arena-patch.js — CrackwithAI Online Battle Arena v3.2
 * ═══════════════════════════════════════════════════════════════════
 *  FEATURES:
 ...
 *
 *  IMPROVEMENTS v3.2:
 *  ✅ Player list displays immediately in battle room waiting area
 *  ✅ Countdown 3-2-1 visible to ALL players (not just admin)
 *  ✅ Each user has independent question progression
 *  ✅ When one user answers, others are NOT forced to skip
 *  ✅ Admin analytics tracks: users left, XP, correct/wrong, attempts
 *  ✅ Timer optimized (100ms ticks, no lag)
 *  ✅ Questions load instantly without freezing
 *  ✅ Results show only when ALL users complete their battle
 * ═══════════════════════════════════════════════════════════════════
 */
...
(function (global) {
  ...
  function hasBattlePlan() {
    ...
    let myUid = global._firebaseAuth?.currentUser?.uid || 
  ...
  */
  ...
  window.BA = {
    ...
    async _pollGameBattle(battleId) {
      ...
      const now = Date.now();
      if (now - this._lastPollTime < 100) return;
      ...
      try {
        ...
        if (data.status === 'generating') {
          ...
          // Reset countdown flags completely when moving to active
          this._countdownShown = false;
          ...
          // CRITICAL FIX: Skip render if progress hasn't changed AND still answering
          ...
          // DEBOUNCE: Only render active quiz if enough time passed
    ...
    _handleCountdown(data, battleId) {
      // CRITICAL FIX: Check if countdown overlay already exists in DOM
      ...
      // Show fullscreen overlay with HIGHEST z-index to ensure visibility
      const overlay = document.createElement('div');
      overlay.className = 'ba-countdown-overlay';
      ...
      const tick = () => {
        ...
        if (remaining > 0) {
          ...
          setTimeout(() => {
            ...
            // Reset flags ONLY after overlay is removed
            this._countdownShown = false;
            this._countdownInProgress = false;
            // Force immediate poll after countdown ends
            this._pollGameBattle(battleId);
            // Resume polling with normal interval
    ...
    _renderActiveQuiz(battle) {
      ...
      // FIX: CRITICAL - Use ONLY this user's individual progress, NOT global question index
      const userProgress = (quiz.userProgress && quiz.userProgress[myUid]) || 0;
      const qi = userProgress;  // This user's current question index
      const questions = battle.questions || [];
      
      // ✅ FIX: Each user gets their own questions - no global skipping
      // Check if THIS USER finished (based on their progress)
      ...
      if (!questions || !Array.isArray(questions) || questions.length === 0) {
        ...
        setTimeout(() => this._pollGameBattle(battle.id || this._activeBattleId), 500);
        return;
      }
      ...
      // FIX: Check THIS USER's specific answer, not shared key
      const myAnswerKey = 'user_' + myUid + '_q' + String(qi);
      const answered = (quiz.answers && quiz.answers[myAnswerKey]) || null;
      const iAnswered = answered !== null;

      const battleId = battle.id || this._activeBattleId;
      const questionStartedAt = quiz.questionStartedAt || battle.startedAt || Date.now();

      // ── HASH GUARD: skip full DOM re-render if same question + answer state ──
      // OPTIMIZATION: Include user progress to detect question changes
      const renderHash = myUid + '|' + qi + '|' + (answered ? 'answered' : 'open') + '|' + JSON.stringify(quiz.xp || {});
      const hashChanged = this._lastRenderHash !== renderHash;
      const questionChanged = battleState.lastQuestionIndex !== qi;
      
      // CRITICAL: If hash is same and question timer exists, skip re-render
      if (!hashChanged && !questionChanged && document.getElementById('ba-qtimer-fill')) {
        // Only update XP pill (cheaply) and let timer keep running
        const xpPill = document.querySelector('.ba-quiz-xp-pill');
        if (xpPill) xpPill.textContent = '⚡ ' + (quiz.xp && quiz.xp[myUid] ? quiz.xp[myUid] : 0) + ' XP';
        
        // OPTIMIZATION: Update existing XP board if it changed
        const xpBoardHTML = this._renderXPBoard(quiz, battle.playerNames);
        if (xpBoardHTML) {
          const existingBoard = document.querySelector('.ba-xp-board');
          if (existingBoard && existingBoard.parentNode) {
            existingBoard.outerHTML = xpBoardHTML;
          }
        }
        return;  // Skip full re-render
      }
      
      // CRITICAL: Mark that we're rendering this question (prevents flicker)
      this._lastRenderHash = renderHash;
      battleState.lastQuestionIndex = qi;
...
(function (global) {
  ...
  // (already set above as global.CosmeticsShop — also expose on window explicitly)
  window.CosmeticsShop = global.CosmeticsShop;
  ...
  console.info('[BattleArena] v3.2 — Player lists + Per-user questions + Admin analytics + Timer optimization loaded');
