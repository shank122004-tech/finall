/**
 * battle-arena-patch.js — CrackwithAI Online Battle Arena v3.2.1
 * ═══════════════════════════════════════════════════════════════════
 *  FEATURES:
 ...
 *
 *  IMPROVEMENTS v3.2.1:
 *  ✅ FIXED: Countdown 3-2-1 lag — replaced nested setTimeout with requestAnimationFrame
 *  ✅ FIXED: Battle not starting after countdown — improved polling synchronization
 *  ✅ FIXED: Race conditions between countdown and polling — prevented re-entry
    ...
    _handleCountdown(data, battleId) {
      ...
      const tick = () => {
        ...
        if (remaining > 0) {
          ...
          // FIXED: Use requestAnimationFrame for smooth 60fps rendering instead of setTimeout(tick, 100)
          ...
          setTimeout(() => {
            ...
            this._countdownShown = false;
            this._countdownInProgress = false;
            // Force immediate poll after countdown ends to pick up active status from Firestore
            this._pollGameBattle(battleId);
            // Resume polling with normal interval after brief delay to prevent race
            setTimeout(() => {
      ...
      // FIXED: Start tick immediately with requestAnimationFrame for smooth animation
      this._countdownAnimationFrameId = requestAnimationFrame(tick);
    },
